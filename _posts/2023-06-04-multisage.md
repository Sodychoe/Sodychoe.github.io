...
GCN 의 단점은 전체 그래프를 입력해야 한다는 점이다, GraphSAGE 는 이를 해결하기 위해 각 레이어마다 고정된 수의 이웃을 샘플링한다
이를 집계하여 업데이트한 아웃풋은 식 (2) 처럼 계산된다.

![](https://user-images.githubusercontent.com/113276452/243671227-b596b59e-11fa-4091-8025-0e124c3f2d7c.png)

GCN 의 capacity + GraphSAGE 의 scalability 를 온전히 사용하기 위해 PinSAGE 가 개발되었다.
이는  관련된 pin 추천 테스크를 위한 모형으로, 실제 추천 시스템 모형을 다루기 위해 여러가지 테크닉들을 사용하였다.

그중에서 주요한 하나는 max-margin ranking 에 기반한 triplet-wise 최적화 목적함수에 있다
여기서 

- $$\delta$$ : margin 을 의미하는 하이퍼 파리미터.
- $$v_q, v_p, v_n$$ : 쿼리 노드, positive 노드, negative 노드이다.
...
타겟 노드와 컨텍스트 노드에 대한 raw feature 가 $$\mathbf{x_t}, \mathbf{x_c}$$ 일 떄,
