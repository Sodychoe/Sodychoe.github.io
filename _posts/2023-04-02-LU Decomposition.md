...
상수를 곱하는 연산이나, 다른 행에 더해주는 연산에 해당하는 기본 행렬의 역행렬을 구해보면 

실제로 성립한다. (예를 들어 , 한 행에 0이 아닌 상수를 곱하는 기본행렬의 역행렬은 그 상수의 역수를 곱하는 기본행렬이다.)

자명하지 않은 부분은 행렬 교환하는 기본행렬인데 이는 Lower triangular 가 아닐 수 있다.

이처럼 행 교환이 포함된 경우 PLU 분해라고 하여 별도로 구분한다.
