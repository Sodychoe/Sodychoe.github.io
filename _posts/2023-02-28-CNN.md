...
[2] 는 Ian Goodfellow 의 [Deep Learning] 책에 있는 합성곱 연산을 설명하는 그림이다. 2차원 형태의 이미지가 입력되었을 때, (우리가 학습시켜야 하는) 핵(kernel)과 이미지가 합성곱 연산을 수행하고 있다. 그 모양은 핵이 특정 간격만큼 움직이면서 각 픽셀과 내적(Inner Product)를 수행하는 것이다.  
